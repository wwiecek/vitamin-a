...
% This is now wrong:
% While meta-analyses have concluded that the average risk ratio of supplementation is 0.76 and thus recommend universal supplementation (Mayo-Wilson et al 2011, Imdad et al 2022). 
...
% arguments from our intro suggest there should be substantial improvements in estimation from using RE models when the true underlying effects are heterogeneous. here are some simulations that prove that. 
...
%WW: isn't this about SEs in relation to heterogeneity in true effects? Easy check is to scale heterogeneity up and down, then see if the results stay constant. We should then say if this is the case!
%RM: i think you may be right. i'm happy for you to spend some time checking this and put it in, but since this is a perpetual pre-print, we could also just footnote this and move on with our lives 
...
%WW: how do we support the claim about the normal fitting well? for now commenting it out
...
Using the Imdad et al.\ (2022) dataset, 
After adjusting for selective reporting with a selection (weight–function) model, the pooled point estimate attenuates and the 95\% interval widens further. In contrast to the latest FE–based meta–analysis, our RE and bias–adjusted analyses therefore temper certainty: they preserve the direction of benefit while avoiding the overconfidence induced by assuming a common effect.\footnote{Insert a sentence with your actual numbers: e.g., “the RE 95\% interval is $x$ times wider than FE; the bias–adjusted interval is $y$ times wider than FE.”}
...
However, adjusting for publication bias the reduction is only 12\% and the interval remains wide compared to FE ($RR = 0.88$, 95\% CI 0.76 to 1.01).
...
\bibitem{andrewskasy2019} Andrews, I., \& Kasy, M. (2019). ``Identification of and Correction for Publication Bias''. \emph{American Economic Review} 109(8), 2766--2794.
...
\bibitem{givewell} GiveWell (2024). ``Vitamin A Supplementation.'' Published April 2024; \url{https://www.givewell.org/international/technical/programs/vitamin-A} (Accessed 27 Oct 2025).

\bibitem{hedges1992} Hedges, L. V. (1992). ``Modeling Publication Selection Effects in Meta-Analysis''. \emph{Statistical Science} 7(2), 246--255.

\bibitem{hedges1998} Hedges, L. V., \& Vevea, J. L. (1998). ``Fixed-and random-effects models in meta-analysis''. \emph{Psychological methods}, 3(4), 486.
...
A\&K-adjusted, all data & -0.13 (-0.28, 0.01) & 0.08 (0.00, 0.21) & --- & ---\\
A\&K-adjusted, no DEVTA & -0.16 (-0.28, -0.04) & 0.06 (0.00, 0.25) & --- & ---\\
\bottomrule
\end{tabular}}
\end{table}
