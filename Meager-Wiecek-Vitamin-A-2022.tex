...
%WW: any differences between FE frequentist vs FE Bayesian?
We compare the theoretical properties and empirical performance of classical fixed-effects and Bayesian random-effects meta-analysis models on simulated bodies of evidence. The Bayesian hierarchical model either matches or substantially outperforms the fixed effects method in terms of mean squared error and is robust to misspecification of the likelihood. Applied to the vitamin A studies, the hierarchical model estimates that supplementation has a risk ratio of 0.77, rather than the fixed effects estimate of 0.89. 
%WW update values
There is substantial underlying heterogeneity across studies: 71\% of the cross-site variation in the estimates is attributable to genuine differences in treatment effects ($I^2 = 0.29$). 
%WW check bounds on I^2
Fixed effects thus underestimates both the reduction in mortality and the uncertainty surrounding the general effect of supplementation. 
...
%WW: I remove this strong claim, since it may need some sort of proof:
...
%WW: simulations need re-running with appropriate number of studies (not Awasthi et al. but Mayo-Wilson et al.)
...
% \begin{equation}
% \begin{aligned}
% \hat{\tau}_k^s &\sim N(\tau^s_k, \hat{se}_k^s) \; \forall \; k\\
% \tau_k^s &\sim N(\tau_k^s, (\sigma_{\tau}^s)^2) \; \forall \; k\\
% \end{aligned}
% \end{equation}
...
%WW: let's discuss: isn't this about SEs in relation to heterogeneity in true effects? Easy check is to scale heterogeneity up and down, then 
%see if the results stay constant. We should then say if this is the case!
...
The above suggests that BHM is appropriate methodology to aggregate the evidence on the effect of Vitamin A supplements on all-cause child mortality. The remainder of this paper performs this analysis and compares the results to the FE method employed by Awasthi et al (2013). 
...
%For the vitamin A literature, the empirical relative risk (RR) is the key object reported and used in the meta analyses in Imbdad et al (2011) and Awasthi et al (2013). The relative risk statistic is defined:
...
This paper studies the same set of papers as Mayo-Wilson et al. (2011) to allow for direct comparison. Hence, we fit this model to the data shown in Table~\ref{summary table}. As it is not easy to interpret a log standard error, we also display the upper and lower bounds of the confidence intervals implied by this log(se). 

We use weakly informative priors centered at zero (no impact of the intervention):
...
We fit both FE and RE models. In addition to estimating the mean and standard deviation (in case of RE) of the effect, we also calculate its p.p.d. 

%WW: This goes earlier???
%This paper studies the same set of papers as Awasthi et al (2013) to allow for direct comparison. Hence, we fit this model to the data shown in figure~\ref{summary table}. As it is not easy to interpret a log standard error, we also display the upper and lower bounds of the confidence intervals implied by this log(se). 
...
The results for Bayesian models with fixed and random effects are reported in Table \ref{baggr-models-tab} and Figure \ref{baggr-density}. For the BHM of all studies we find the mean RR is 0.74 (95\% interval from 0.58 to 0.88), compared to 0.88 under FE model (95\% interval from 0.83 to 0.93), similar to the value reported by Mayo-Wilson et al.
The effect under RE model is more diffuse than under the FE model, but both models suggest that in the analysed studied vitamin A supplements have on average considerably reduced child mortality. The FE method gives a higher and more precise estimate of $RR$ because it does not take into account the heterogeneity across sites. 
...
This heterogeneity is also reflected in the posterior predictive distribution of risk ratios for the RE model. It has a mean of 0.77, with 95\% interval ranging from 0.4 to 1.3. (For the FE model it is by definition the same as the average effect.) This indicates that while we can be confident in the vitamin A effects historically, the probability of true effect in a new implementation does include possibility of no effect in some settings: we calculate probability of RR being below 1 at 88\%.\footnote{SOME TEXT ON THE TRUE DISTRIBUTION BEING ASYMMETRIC: WE DON'T BELIVE 5\% CHANCE OF 30\% INCREASE IN MORTALTIY!}
...
The controversy in the vitamin A literature arose in part because of disagreements over the validity of using fixed effects methods to aggregate the evidence, and the suitability of the random effects alternative in this setting. Although Awasthi et al (2013) argued that random effect models "conceal the reliability" of large trials and "give undue weight to small trials," the analysis presented in this paper shows that there are strong theoretical and empirical reasons to prefer the random effects methods, implemented using Bayesian hierarchical models. While fixed effects models are optimal under homogeneous treatment effects, random effects models are optimal when the effects have underlying heterogeneity. When neither model describes the true data-generating structure, Monte Carlo simulation results show that the Bayesian hierarchical model either matches or outperforms the FE estimator by a wide margin in terms of MSE.

Using the same input data as the Awasthi et al (2013) meta-analysis exercise, a Bayesian hierarchical analysis estimates a typical reduction in the risk of child deaths of 23\%. Substantial heterogeneity is detected in treatment effects, with 71\% of the observed variation attributed to genuine underlying differences across sites. Thus, the Bayesian hierarchical model results are robust to the inclusion or exclusion of the DEVTA results: despite its precision, it is only one study in a heterogeneous literature. Fundamentally, in the presence of heterogeneous treatment effects, precision is not a signal of generalizability. Because the FE estimator cannot distinguish these two concepts, it underestimates the reduction in mortality risk, and overestimates the confidence we should have in this estimate. This is in keeping with previous work showing the FE estimator typically overstates the precision of meta-analysis findings (Schmidt, Oh and Hayes, 2011) and thus the RE estimator is generally preferable when seeking to generalize beyond the set of studies in hand (Borenstein et al, 2010). This analysis also confirms the advantages of the Bayesian approach to meta-analysis (Smith, Spiegelhalter, and Thomas, 1995).

When properly aggregated, the evidence shows that the general impact of vitamin A supplements on all-cause child mortality is substantial. However, the evidence also suggests important differences in treatment effects across studies, and caution should be used before extrapolating from the studies in this literature. Given the biological mechanisms, vitamin A supplements should have a much larger effect on mortality when the population has a high rate of severe vitamin A deficiency, and this alone could account for the observed heterogeneity. However, study design could play an equally important role, given the major differences in delivery mechanism, compliance and monitoring across studies. Future work could explore the sources of this cross-site heterogeneity, and thus improve the effectiveness of the supplements in low-impact settings or target supplement programs more effectively to areas that need them most.
...
\bibitem{bda} Gelman, A., John B. Carlin, Hal S. Stern \& Donald B. Rubin (2004) "Bayesian Data Analysis: Second Edition", Taylor \& Francis.
...
\begin{table}
\caption{Comparison of results of all four meta-analysis models fitted to our data.}
\label{baggr-models-tab}
\centering
\resizebox{\linewidth}{!}{
\begin{tabular}[t]{llll}
\toprule
  & Hypermean & Hyper-SD & p.p.d.\\
\midrule
\textbf{Partial, all data} & \textbf{-0.31 (-0.54, -0.13)} & \textbf{0.27 (0.1, 0.53)} & \textbf{-0.31 (-0.95, 0.28)}\\
Partial, no DEVTA & -0.34 (-0.58, -0.16) & 0.25 (0.038, 0.56) & -0.34 (-1, 0.24)\\
Full, all data & -0.13 (-0.18, -0.068) & 0 (0, 0) & -0.13 (-0.18, -0.068)\\
Full, no DEVTA & -0.28 (-0.37, -0.18) & 0 (0, 0) & -0.28 (-0.37, -0.18)\\
\bottomrule
\end{tabular}}
\end{table}
