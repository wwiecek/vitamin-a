...
In more complicated models, the ML estimation quickly becomes a challenging multivariate optimisation problem that requires inversion of large matrices to calculate standard errors. In these more complex cases, the ``Empirical Bayes'' MLE is performed using a 2-step procedure that does not accurately quantify the uncertainty in the unknowns. In general, frequentist random-effects meta-analytic estimators such as MLE are known to underestimate heterogeneity.\footnote{In some cases, these estimators are known to "snap to boundary" (estimating heterogeneity parameter $\tau=0$) even when heterogeneity is present, but highly uncertain. This occurs particularly often (but not only) when the number of studies in a meta-analysis is low.}
